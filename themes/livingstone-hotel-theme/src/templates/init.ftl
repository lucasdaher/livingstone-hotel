<#assign theme_display=themeDisplay portlet_display=portletDisplay layoutSet=layout.getLayoutSet()
	theme_timestamp=themeDisplay.getTheme().getTimestamp() theme_settings=themeDisplay.getThemeSettings()
	root_css_class=languageUtil.get(locale, "lang.dir" ) css_class=htmlUtil.escape(bodyCssClass!) css_class=css_class
	+ " " + htmlUtil.escape(theme_display.getColorScheme().getCssClass()) + " yui3-skin-sam"
...
<#assign css_folder=theme_display.getPathThemeCss() images_folder=theme_display.getPathThemeImages()
	javascript_folder=theme_display.getPathThemeJavaScript() templates_folder=theme_display.getPathThemeTemplates()
	full_css_path=fullCssPath full_templates_path=fullTemplatesPath
	...
	company_id=company.getCompanyId() company_name=htmlUtil.escape(company.getName())
	company_logo=htmlUtil.escape(theme_display.getCompanyLogo()) company_logo_height=theme_display.getCompanyLogoHeight()
	company_logo_width=theme_display.getCompanyLogoWidth() company_url=theme_display.getURLHome() />

<#if !request.isRequestedSessionIdFromCookie()>
	<#assign company_url=portalUtil.getURLWithSessionId(company_url, request.getSession().getId()) />
</#if>

<#assign time_zone=user.getTimeZoneId() is_login_redirect_required=portalUtil.isLoginRedirectRequired(request)
	is_signed_in=theme_display.isSignedIn() group_id=theme_display.getScopeGroupId() />

<#if !user_initialized??>
	<#assign is_default_user=user.isDefaultUser() is_female=user.isFemale() is_male=user.isMale()
		is_setup_complete=user.isSetupComplete() language=locale.getLanguage() language_id=user.getLanguageId()
		user_birthday=user.getBirthday() user_comments=user.getComments() user_email_address=user.getEmailAddress()
		user_first_name=user.getFirstName() user_greeting=htmlUtil.escape(user.getGreeting()) user_id=user.getUserId()
		user_last_login_ip=user.getLastLoginIP() user_last_name=user.getLastName() user_login_ip=user.getLoginIP()
		user_middle_name=user.getMiddleName() user_name=user.getFullName()
...
<#if show_home>
	<#assign home_text=languageUtil.get(locale, "home" ) home_url=htmlUtil.escape(theme_display.getURLHome()) />
...
<#assign selectable=theme_display.isTilesSelectable() is_maximized=layoutTypePortlet.hasStateMax() page_javascript_1=""
	page_javascript_2="" page_javascript_3="" page=layout is_first_child=page.isFirstChild()
	is_first_parent=page.isFirstParent() is_portlet_page=false />
...
<#assign site_name=htmlUtil.escape(page_group.getDescriptiveName()) community_name=site_name
...
<#assign css_class=css_class + " " + site_type
	...
	community_default_private_url=site_default_private_url site_default_url=site_default_public_url
	community_default_url=site_default_url />

<#if layout.isPrivateLayout()>
	<#assign site_default_url=site_default_private_url community_default_url=site_default_url />
</#if>
...
<#assign layout_friendly_url=layout.getFriendlyURL() portlet_id=paramUtil.getString(request, "p_p_id" ) />

<#if validator.isNotNull(portlet_id) && layout.isSystem() && !layout.isTypeControlPanel() &&
	stringUtil.equals(layout_friendly_url, "/manage" )>
...
<#assign logo_css_class="logo" use_company_logo=!layoutSet.isLogo() site_logo_height=company_logo_height
...
<#if theme_settings["show-site-name-default"]??>
	<#assign show_site_name_default=getterUtil.getBoolean(theme_settings["show-site-name-default"]!"",
		show_site_name_supported) />
	<#else>
		<#assign show_site_name_default=show_site_name_supported />
</#if>

<#assign show_site_name=getterUtil.getBoolean(layoutSet.getSettingsProperty("showSiteName"), show_site_name_default)
	site_logo=company_logo logo_description="" />
...
<#if navItems??>
	<#assign nav_items=navItems has_navigation=(nav_items?size> 0)
		/>
</#if>
...
<#assign show_my_sites=user.hasMySites() show_my_places=show_my_sites />

<#if show_my_sites>
	<#assign my_sites_text=languageUtil.get(locale, "my-sites" ) my_places_text=my_sites_text />
</#if>

<#assign dir_include="/html" body_bottom_include="${dir_include}/common/themes/body_bottom.jsp"
	body_top_include="${dir_include}/common/themes/body_top.jsp" bottom_include="${dir_include}/common/themes/bottom.jsp"
...
<#assign date=dateUtil current_time=date.newDate() the_year=current_time?date?string("yyyy") />
